...
chrome.runtime.onInstalled.addListener(() => {
    chrome.storage.sync.get(['loginAction', 'logoutAction', 'reloadDelay'], (data) => {
      ...
      if (data.reloadDelay === undefined) {
        chrome.storage.sync.set({ reloadDelay: 1 });
      }
    });
  });
  ...
  function handleHostnameTabs(hostname: string, action: string, currentTabId: number | undefined) {
      ...
      tabs.forEach((tab) => {
        if (tab.url) {
          ...
          if (tab.id !== currentTabId && tabHostname.includes(hostname)) {
            if (action === 'reload') {
              chrome.tabs.reload(tab.id!, undefined, () => {});
  ...
  function handleNavigationEvent(eventType: string, hostname: string, currentTabId: number) {
    ...
    chrome.storage.sync.get([settingKey, 'reloadDelay'], (data) => {
      ...
      chrome.storage.local.get([`initiator_${currentTabId}`], (result) => {
        ...
        chrome.storage.local.set({ [`initiator_${currentTabId}`]: true }, () => {
          function handleMatchingTabs(hostname: string, initialTabId: number) {
            chrome.tabs.query({}, (tabs) => {
              tabs.forEach((tab) => {
                if (tab.url) {
                  const tabHostname = new URL(tab.url).hostname;
